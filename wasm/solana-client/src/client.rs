...
use {
    ...
    solana_rpc_client_api::{
        ...
        request::{RpcError, RpcRequest, TokenAccountsFilter},
        response::{
...
pub type RpcResult<T> = Result<Response<T>, ClientError>;
...
impl WasmClient {
    ...
    pub fn new_with_commitment(url: String, commitment_config: CommitmentConfig) -> Self {
        Self {
            provider: Provider::Http(HttpProvider::new(url)),
    ...
    pub fn new_with_timeout(url: String, timeout: Duration) -> Self {
        Self {
            provider: Provider::Http(HttpProvider::new_with_timeout(url, timeout)),
            config: RpcClientConfig::with_commitment(CommitmentConfig::default()),
            #[cfg(feature = "pubsub")]
            ws: crate::pubsub::WasmWebSocket::new(endpoint),
        }
    }
    ...
    pub async fn get_block_time(&self, slot: Slot) -> ClientResult<UnixTimestamp> {
        ...
        response
            .map(|result_json: Value| -> Result<UnixTimestamp, ClientError> {
